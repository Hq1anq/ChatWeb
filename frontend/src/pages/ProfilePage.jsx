...
const ProfilePage = () => {
  const { user, isUpdatingProfile, updateProfilePic, updateProfileBio, updatePassword } = useAuthStore()
            ...
            <div className="flex flex-col items-center mb-6">
              <div className="avatar mb-4">
                <div className="w-24 h-24 rounded-full ring ring-primary ring-offset-base-100 ring-offset-2 overflow-hidden">
                  <img
                    src={user?.profilepic ? `${import.meta.env.VITE_SERVER_URL}${user.profilepic}` : '/avatar.png'}
                    alt="User Avatar"
              ...
              <label className={`btn btn-sm btn-outline btn-primary cursor-pointer ${isUpdatingProfile ? 'btn-disabled opacity-50' : ''}`}>
                {isUpdatingProfile ? <Loader2 className="animate-spin w-4 h-4" /> : <Edit2 size={14} />}
                <span>{isUpdatingProfile ? 'Đang tải...' : 'Thay đổi Avatar'}</span>
            ...
            <div className="card-actions justify-end mt-6">
              {isEditing ? (
                <button 
                  ...
                  disabled={isUpdatingProfile}
                >
                  {isUpdatingProfile ? <Loader2 className="animate-spin w-4 h-4" /> : <Save size={18} />}
                  Lưu thay đổi
                </button>
              ) : (
                <button
          ...
          <div className="card-body">
            ...
            <div className="card-actions justify-end mt-6">
              <button 
                ...
                disabled={isUpdatingProfile || !passwords.currentPassword || !passwords.newPassword}
              >
                {isUpdatingProfile ? <Loader2 className="animate-spin w-4 h-4" /> : "Đổi mật khẩu"}
              </button>
            </div>
          </div>
        </div>
